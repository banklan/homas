<template>
    <v-container>
            ...
            <v-col cols="12" md="5">
                <v-card light raised outlined elevation="4" min-height="400" class="scroll" width="100%">
                    <template v-if="!changePic">
                        <v-img v-if="authAdmin.picture" :src="`/images/profiles/admins/${ authAdmin.picture }`" width="100%" height="350" transition="scale-transition"></v-img>
                        ...
                        <v-card-text>
                            <table class="table table-hover table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Last Updated: </th>
                                        <td>{{ authAdmin.updated_at | moment('Do MMM, YYYY, H:mma') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </v-card-text>
        ...
        <v-snackbar :value="updatedAdminProfile" :timeout="3000" top color="green darken-1 white--text">
            Your profile was updated successfully.
            <v-btn text color="white--text" @click="updatedAdminProfile = false">Close</v-btn>
        </v-snackbar>
...
export default {
    ...
    computed: {
        ...
        updatedAdminProfile(){
            return this.$store.getters.updatedAdminProfile
        }
    },
    methods: {
        openEditForm(){
            this.openEdit = true
            this.edit.first_name = this.authAdmin.first_name
            this.edit.last_name = this.authAdmin.last_name
