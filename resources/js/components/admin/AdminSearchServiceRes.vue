                            ...
                            <table class="table table-condensed table-striped table-hover">
                                ...
                                <tbody class="table_list">
                                    <tr v-for="(service, i) in services" :key="service.id">
                                        ...
                                        <td><v-btn small icon color="red darken-2" @click="confirmDel(service, i)"><v-icon>delete_forever</v-icon></v-btn></td>
                                    </tr>
                                </tbody>
                            </table>
                        </v-card-text>
                    ...
                    <template v-else>
                        <v-card-text>
                            <v-alert type="info" border="left" class="mt-8">
                                Your search for {{ $route.query.q }} returned no service.
                            </v-alert>
                        </v-card-text>
                    </template>
                </v-card>
...
export default {
    ...
    methods: {
        getResult(){
            this.isLoading = true
            axios.post(this.api + '/auth-admin/admin_search_for_services', {
