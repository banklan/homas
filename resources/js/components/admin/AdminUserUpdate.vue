        ...
        <v-row justify="start" class="mt-5">
            <v-col cols="12" md="8">
                ...
                <v-card v-else light raised outlined elevation="4" min-height="400">
                    <v-card-title class="justify-center subtitle-1 admin white--text">Update User Details</v-card-title>
                    ...
                    <v-card-actions class="justify-center pb-4">
                        <v-btn width="40%" text large color="red darken-2" :to="{name: 'AdminUserDetail', params:{id: user.id} }">Cancel</v-btn>
                        <v-btn width="40%" dark large color="admin" @click="updateUser" :loading="isUpdating">Save Update</v-btn>
                    </v-card-actions>
                </v-card>
            </v-col>
        </v-row>
            ...
            this.$validator.validateAll().then((isValid) => {
                if(isValid) {
                    ...
                    axios.post(this.api + `/auth-admin/admin_update_user/${this.id}`, {
                        ...
                        this.$router.push({name: 'AdminUserDetail', params: {id: res.data.id}})
                    }).catch(() => {
                        this.isUpdating = false
