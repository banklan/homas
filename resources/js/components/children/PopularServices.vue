            ...
            <template v-if="services.length > 0">
                <v-col cols="12" md="4" v-for="service in services" :key="service.id">
                    <v-card light elevation="4" min-height="400" class="mt-5 mb-3" :to="{name: 'ServiceShow', params:{id: service.id, slug: service.slug}}">
                        ...
                        <v-card-text class="text-center body-1 grey--text mt-n3"><v-icon color="primary">place</v-icon> {{ service.city }} | <v-icon color="primary">layers</v-icon> {{ service && service.category.name }} </v-card-text>
